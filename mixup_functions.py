...
def Train(model, train_DL, criterion, optimizer, class_correct_counts, class_total_counts):
    ...
    for images, labels_a, labels_b, lam in train_DL:
        images = images.to(DEVICE)
        labels_a = labels_a.to(DEVICE)
        labels_b = labels_b.to(DEVICE)
        y_hat = model(images)
        loss = lam * criterion(y_hat, labels_a) + (1 - lam) * criterion(y_hat, labels_b)
        
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        
        loss_b = loss.item() * images.shape[0]
        rloss += loss_b
        
        correct_top1, correct_top5 = calculate_ranking(y_hat, labels_a)
        total_top1 += correct_top1 * images.size(0)
        total_top5 += correct_top5 * images.size(0)
        
        _, predicted_classes = torch.max(y_hat, 1)
        for i in range(len(labels_a)):
            if predicted_classes[i] == labels_a[i]:
                class_correct_counts[labels_a[i]] += 1
            class_total_counts[labels_a[i]] += 1
